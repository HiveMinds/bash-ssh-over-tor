...
can_locally_ssh_into_follower_over_wan_or_lan() {
  ...
  sshpass -p "$follower_ubuntu_password" ssh -o ConnectTimeout=5 "$follower_ubuntu_username@$follower_local_ip" echo "SSH test connection" >/dev/null 2>&1
  status="$?"
...
assert_can_locally_ssh() {
  ...
  can_locally_ssh_into_follower_over_wan_or_lan "$follower_ubuntu_username" "$follower_local_ip" "$follower_ubuntu_password"
